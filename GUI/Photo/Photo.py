...
class Photo():
        ...
        def getKeyPoints(self):
            ...
            keypoints_photo = corner_peaks(corner_harris(self.photo_path), min_distance=20)
            brief = BRIEF()
            brief.extract(self.photo,keypoints_photo)
            keypoints_photo = keypoints_photo[brief.mask]
            descriptor_photo = brief.descriptors
            
            #para comparar los descriptores de las imagenes
            #matchesDescriptors = match_descriptors(descriptor_photo, descriptor_imgGray)
            
            return descriptor_photo
